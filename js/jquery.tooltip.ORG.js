			...
			if(parent.options.floatType == 'staticTop'){ //above the element
				...
				if (navigator.userAgent.indexOf('AppleWebKit') != -1){
					...
					if (offsetX !== -1 && offsetY !== -1) { 
						$('#pm_marker_tooltip').css("top", ( ($el.offset().top - $el.height()) + (offsetY / 5) )).css( "left", $el.offset().left - (tipWidth / 2) + offsetX ).css("opacity" , 1).css("marginTop" , 0);
					} else {
						$('#pm_marker_tooltip').css("top", ( ($el.offset().top - $el.height()) + 5 )).css( "left", $el.offset().left - (tipWidth / 2) + 10 ).css("opacity" , 1).css("marginTop" , 0);		
					}
					...
					if (offsetX !== -1 && offsetY !== -1) { 
						...
						$('#pm_marker_tooltip').css("top", ( ($el.offset().top - $el.height()) + -25 )).css( "left", $el.offset().left - (tipWidth / 2) + 10 ).css("opacity" , 1).css("marginTop" , 0);		
					}
